...
- **Kerning** — the legacy `kern` table (0.6.5): `rekha_kern_pair` / `_px` and `rekha_char_kern`,
  over both incompatible forms that share the tag. MEASURED against fontTools on all 16 `kern`
  fonts of the dev host: **163,183 pairs identical**, and 8,000 probes for pairs that are not
  there all correctly 0. ⚠ A modern face keeps its kerning in **GPOS**, which rekha does not read
  yet — such a font answers 0, which is the unkerned spacing and not a wrong number.
