...
into the glyph zone with. This release is the loader — simple and composite glyphs into zone 1 in
place, the four phantom points, `IUP`, the glyph program under FreeType's glyph-range rules,
composites assembled FreeType's way — and the public call that answers a **HINTED OUTLINE**:
`rekha_hint_glyph(ctx, gid)`, then `rekha_hint_outline` / `rekha_hint_to_sdpath` and
`rekha_hint_advance_px`. **0.8.x closes here.** Every instruction of the machine runs; the
interpreter produces no `REKHA_ERR_UNSUPPORTED` at all any more.

⭐ **FreeType IS THE ORACLE ON REAL GLYPHS NOW.** `programs/hint_glyph_vectors.cyr` (generated by
`scripts/hint_glyph_vectors.py` through `scripts/ftshim.py`) holds FreeType 2.14.3's own hinted
outlines of the embedded face — **233 rows over 31 glyphs, 5,372 points**: the 18 characters of
0.8.1's table at 8, 9, 11, 12, 13, 16, 18, 24, 32 and 48 ppem, eleven composites at 12, 16 and
24 (é with `USE_MY_METRICS` and a program that moves points, Å inheriting A's program-moved pp2,
½ whose composite program runs `IUP`, ¼ with no program at all, NBSP built from an empty glyph,
ͼ and ͣ whose offsets land on a negative half pixel), the space and a program-less simple glyph
— every point's position as the glyph slot reported it, its tag byte, the contour ends and
`slot->advance.x`, every row also loaded clean under `FT_LOAD_PEDANTIC`. Group P of
`programs/hint_test.cyr` runs them through the public call and **rekha agrees point for point,
tag for tag, end for end and on the advance — 233 of 233.**

### The headline: the whole face, and 165 + 50 other faces

| what | loads | differ | refused where FreeType's pedantic mode loads | refused where pedantic ALSO fails |
|---|---|---|---|---|
| `scripts/hint_glyph_diff.py sweep` — Liberation Sans, 2,620 glyphs × 10 ppems (8 .. 48) | **26,200** | **0** | **0** | 0 |
| the same at 5, 6, 7, 100, 201 and 500 ppem | 15,720 | 0 | 0 | 0 |
| `corpus /usr/share/fonts --ppems 12 --one-per-family` — 165 hinted faces, the Regular of each family | **2,593,954** | **0** | **0** | 595 |
| `corpus` over 50 faces chosen for what one Regular per family cannot see — Bold / Italic / Thin / Black / Condensed / Extended, Hangul, Thai, Hebrew, Greek, Cyrillic, the host's only hinted `vmtx` faces — at 9, 11, 13 and 20 ppem | **2,812,516** | **0** | **0** | 17,158 (0.61%) |
| every glyph 0..4 of all 167 unit-row fonts at the row's ppem | 674 | 0 | 0 | — |

⚠ **The last column is the refuse-don't-guess policy, and it is no longer unmeasured.** Every
one of those loads is a glyph FreeType's DEFAULT mode hints — by reading a control value past
the end of the `cvt ` as 0 (`RCVT index`, all but four), by dropping a point outside the zone
(`IP point`, LiberationMono's gid 2215 '♫', one per ppem) or by reading a short stack as zeros
(`stack underflow`, DejaVuSansM Bold's 'Д' at 13 ppem, which FreeType fits to 7 px in an 8-px
monospace) — and its pedantic mode refuses. The 12-ppem run put it at 595 loads: Mononoki Nerd
Font ×3 (197 each), Literation Mono Nerd Font ×3 and LiberationMono-Regular (1 each). The
four-ppem run shows it is ppem-periodic: **AdwaitaMono-Regular refuses 2,624 of its 8,818 glyphs
at 9 of 25 ppems** (6, 9, 13, 16, 19, 22, 26, 32, 48) and none at the other 16 — all `RCVT` past
the table, all non-ASCII (U+21xx–2Bxx arrows, math, box drawing and braille, U+4Dxx, U+A7xx and
`.notdef`; 2,094 simple, 530 composite). No ASCII glyph is refused on any face. The decision is
in *Divergences* below.

Six faces of that class, re-run after the fix round with the size set once per ppem, exactly as
a consumer drives FreeType (ppems 9, 11, 13, 20):

| face | loads | agree | differ | refused | FreeType-pedantic-fails |
|---|---|---|---|---|---|
| AdwaitaMono-Regular | 35,272 | 30,024 | 0 | 0 | 5,248 |
| 0xProtoNerdFont-Italic | 47,428 | 47,356 | 0 | 0 | 72 |
| ArimoNerdFont-BoldItalic | 55,684 | 55,234 | 0 | 0 | 450 |
| CousineNerdFont-Italic | 53,320 | 53,174 | 0 | 0 | 146 |
| DejaVuSansMNerdFont-Bold | 56,780 | 56,779 | 0 | 0 | 1 |
| LiberationMono-Bold | 9,692 | 9,688 | 0 | 0 | 4 |

⚠ **The host has no hinted `gvar` face and no `hdmx` face** (0 of 1,751 `glyf` fonts carry
`hdmx`; AdwaitaSans has `gvar` and no bytecode), so the variable-instance and `hdmx` pins below
are stated, not measured on a face in the wild. A scan of all 1,129 hinted faces found the
deepest composite nesting to be exactly 5 (AgaveNerdFont gid 1441, which loads and agrees), at
most 9 direct components and 17 loads per glyph, and no face understating its `maxp` maxima — so
rekha's caps refuse nothing on this host.

### The synthetic composites: 167 programs on 39 fonts, every row run three times

`scripts/hint_unit_vectors.py` grew what Liberation Sans cannot measure: every unit font now
carries four glyphs (0 empty, 1 the outline, 2 a composite of two glyph-1 components, 3 a nested
composite of 1 and 2; two variants a glyph 4 with a program of its own), and 14 new variants
change the composite records, an `hmtx` entry, `hhea.descender` or add a `vmtx`.
`programs/hint_unit_vectors.cyr` is **167 programs on 39 fonts** (134 on the simple glyph, 33 on
composites; 9,206 lines, 699,713 bytes under a new 900,000-byte cap the emitter asserts). Group K
runs every row **three times**: the simple rows through the loader's fill and the test runner;
every row through `rekha_hint_glyph` on a copy of its font patched to carry the program; and
every row the consumer's way — glyphs 1, 2 and 3 in order on one fresh context — against what
FreeType's one face reported for glyphs 2 and 3 after the same sequence (`hu_expect2_*`,
`hu_expect3_*`, the size set ONCE, `prep` once).

Facts the new rows pin, each from `libfreetype` and not computed: an offset on a negative half
pixel rounds by `FT_PIX_ROUND`'s floor (−96 → −64) under `ROUND_XY_TO_GRID` and stays under it
otherwise; a 2x2 applies to the HINTED F26Dot6 points with `FT_MulFix` per term; a
`SCALED_COMPONENT_OFFSET` offset is `FT_MulFix` by `FT_Hypot` of the matrix column — shear
(128, 64) rounded / (150, 84) unrounded, a full 2x2 (−158, 126), `WE_HAVE_A_SCALE` 0.75
(−170, 56), `WE_HAVE_AN_X_AND_Y_SCALE` (150, 113) — and a record with NEITHER flag is unscaled on
the host build; `SCALED` and `UNSCALED` both set means scaled; point matching, byte- or
word-argued, nested or not, is `cur − cur` unrounded; `USE_MY_METRICS` on an empty component
carries its phantoms un-offset and unrounded (advance 384); a composite of empty components never
runs its program (896, not 960); a `vmtx` tail entry takes the last long entry's height; a simple
component's program runs with `first_point` 0 at base 14 and 28; `WCVTP` / `WS` / `DELTAC1`
write a per-load copy shared by both components and the composite program while `WCVTF` writes
the size's `cvt ` (300 read back in the next load); an `SMD` in a component program does not reach
the composite program (889, not 1,125); `DIV` truncates (46 / −46) where `MUL` rounds (64 / −64).
⭐ The `FT_Vector_Length` CORDIC is ported verbatim and checked against `libfreetype`'s own on
34,318 vectors over the whole signed 32-bit domain, 0 mismatches (an integer square root differs
from it by 1 in 25 of 661,201 samples, which is why it is ported and not approximated).

### The public surface, and the rules a consumer needs

`rekha_hint_glyph(ctx, gid)` → 1 when zone 1 holds what the bytecode intends at this size, 0 when
rekha refused. `rekha_hint_outline(ctx)` → a `RekhaOutline` VIEW living in the context: xs / ys the
zone's cur in F26Dot6, x translated so the left phantom is the origin as FreeType's slot reports
it, the phantoms excluded, on-curve one byte per point — rewritten by every `rekha_hint_glyph`,
`rekha_hint_run_prep` and size change. `rekha_hint_to_sdpath(ctx, ox, oy)` → the view through
`rekha_outline_to_sdpath` at scale 1024 (F26Dot6 × 1024 IS 16.16), a fresh, exactly sized path.
`rekha_hint_advance_px(ctx)` → the right phantom minus the left, rounded as FreeType's base layer
rounds a hinted slot's advance, in WHOLE PIXELS. `rekha_hint_gid(ctx)` → the gid or −1.
`rekha_left_side_bearing(font, gid)` (src/sfnt.cyr) with `hmtx`'s tail rule and the HVAR lsb
delta, through the new `rekha_hvar_lsb_delta(font, gid)` (src/hvar.cyr) — a NULL lsb map answers
0, the spec's rule, unlike the advance map. ⛔ No raw F26Dot6 advance is published: it is a
pre-grid-fit intermediate no FreeType consumer sees.

- ⛔ **`gasp` is the consumer's.** `rekha_hint_glyph` hints whenever asked, exactly as FreeType's
  loader never consults the table; whether to hint at this size at all is `rekha_should_gridfit`'s
  question. The oracle hints Liberation at 8 ppem where `gasp` says smooth.
- ⛔ **Warm the context outside any scoped per-frame `sd_alloc` hook**: `rekha_hint_ctx(font, ppem)`
  is a lazy allocation cached on the handle, and on a variable face load ONE glyph outside the hook
  too — the gvar scratch is allocated by the first varied load. After that a hinted glyph costs
  **0 bytes**: group O pins 'A' and 'é' at 12 ppem, a second size and the refusal paths at 0 B, and
  `rekha_hint_to_sdpath` at exactly the path's own allocations.
- ⛔ **Use `rekha_hint_advance_px` for every glyph of a hinted run.** A glyph program is free to
  move the right phantom: Liberation Sans's 'H' at 16 ppem is **12 px** from
  `rekha_glyph_advance_px` and **11 px** hinted. Pass `ox` / `oy` on WHOLE PIXELS (`n << 16`) or
  the fit the program made is lost.
- ⛔ **After an axis change call `rekha_hint_ctx` again**, at the same ppem if you like:
  `rekha_var_set_axis`, `rekha_var_set_instance` and `rekha_var_reset` mark the cached context
  stale and that call re-runs `prep` for the new instance.

### ⛔ Three kinds of refusal, and `rekha_hint_error` says which

- A **standing `fpgm` / `prep` refusal**, re-raised: zone 1 empty, nothing to draw hinted at this
  size — the consumer draws the run through the unhinted seam.
- A **program refused** — `REKHA_ERR_BAD_HINT` naming the instruction, or `TRUNCATED`
  `"instruction"` / `"push operands"` — in any component's program or the composite's: zone 1
  holds the glyph RELOADED UNHINTED (scaled, no program run anywhere, phantoms unrounded,
  translated all the same), so the outline, the path and the advance still answer, the way
  FreeType answers a glyph whose programs are off. The gid is set and the view valid.
- **Anything structural**, with zone 1 EMPTY and `rekha_hint_gid` −1: `OOM "glyph load cap"`
  (depth over 5, over 64 loads, a cycle, over 16,384 points), `OOM "glyph zone cap"` /
  `"contour cap"` (past `maxp`'s own capacity), `TRUNCATED "glyf header"` / `"endPts"` /
  `"instructions"` / `"points"` / `"component"` / `"composite instructions"`, `BAD_GLYF "endPts"`
  / `"endPts order"` / `"flags"` / `"composite point"`, and `rekha_glyf_span`'s own — `OTHER
  "glyph id"`, `NO_TABLE "loca"` / `"glyf"`, `TRUNCATED "loca"`, `BAD_SFNT "indexToLocFormat"`,
  `BAD_GLYF "loca order"` / `"loca past glyf"`.

In the glyph range `FDEF` and `IDEF` refuse `"FDEF in glyph program"` / `"IDEF in glyph program"`
(FreeType's `DEF_In_Glyf_Bytecode`, in both modes). `rekha_font_error(font)` mirrors the code either
way, and every call invalidates the previous glyph, success or not.

### Added

`rekha_hint_glyph`, `rekha_hint_outline`, `rekha_hint_gid`, `rekha_hint_to_sdpath`,
`rekha_hint_advance_px`, `rekha_left_side_bearing`, `rekha_hvar_lsb_delta` (above); `IUP` (0x30 /
0x31), the last refusal; the glyph-range rules — `WS` / `WCVTP` / `DELTAC` writing a PER-LOAD copy
of the storage area and the control values, shared by every component and the composite program
of one load, `WCVTF` alone writing through to the size's `cvt ` (`rekha_hint_cvt_px` and
`rekha_hint_storage` read the CURRENT pointer); the post-`prep` snapshot of the ten graphics-state
fields every program of every load starts from; `INSTCTRL` bit 1 (no program, no phantom rounding
for the load — the advance is still whole through `_px`) and bit 2 (the defaults, kept until the
next `prep`), tested in `tt_loader_init`'s order; the composite program at scale 1.0 over
`org = orus = cur`; `USE_MY_METRICS` on the hinted path; `rekha_hint_ppem` answering 0 between an
axis change and the next `rekha_hint_ctx`. In `src/glyf.cyr` the simple-glyph decoder is one
shared core (`rekha_glyf_decode_simple`), written once into whichever arrays the caller hands it;
`rekha_gvar_apply_n` lands a tuple's four phantom deltas, applied only on a face without HVAR
(pp1 / pp2) or VVAR (pp3 / pp4), FreeType's rule. `src/error.cyr` says the interpreter no longer
produces `UNSUPPORTED` and lists the codes a hinting refusal can carry.

Scripts: `scripts/hint_ft_vectors.py` is **renamed `scripts/hint_glyph_vectors.py`** and emits the
module rather than a digest table; `scripts/hint_glyph_diff.py` is new — `sweep`, `diff <gid>
<ppem>`, `corpus [DIR|FILE ...] [--ppems] [--one-per-family]`, `mkdump`, `selftest`, through a
throwaway dumper it writes to `build/hint_dump_<pid>.cyr` exactly as `scripts/kern_diff.py` does;
`scripts/ftshim.py` gains `set_size(face, ppem, force)` and **sets the size ONCE per (face,
ppem)** — a shim that re-set it before every load re-ran `prep` per glyph and could never observe
what one glyph program leaves for the next, which is exactly what rekha's context keeps;
`scripts/hint_unit_vectors.py` rounds 4 and 5, with `load_sequence` / `consumer_flow` for the
`hu_expect2_*` / `hu_expect3_*` records; `scripts/hint_vectors.py`'s byte formula updated.

### Fixed — each one measured against FreeType

- ⭐ **`DIV` rounded where FreeType truncates.** `Ins_DIV` is `FT_MulDiv_No_Round`
  (ref2143/ttinterp.c:2657-2664, ftcalc.c:187-206); 0.8.0 and 0.8.1 shipped the rounding form, and
  `MUL` stays `FT_MulDiv`. Found by the differential on **ͼ (gid 822) at 16 ppem**, point 26 at
  201 against FreeType's 265, traced through Liberation's `fpgm` function 37 (`PUSHW -128 DIV`):
  93 / −128 is −46 truncated and −47 rounded, and the accent landed one pixel off. Now a unit row
  (`div_trunc`, with `mul_round` beside it).
- **A simple component's `SHZ` shifted its own phantom points.** The nested-composite mirror of
  FreeType's `first_point` was derived from the sub-zone's offset, so it applied to a SIMPLE
  component's program too, where FreeType's `first_point` is 0 (ref2143/ttgload.c:914). The `shz`
  row's gid 2: point 27 at 921 against FreeType's 896 at 12 ppem, and a `USE_MY_METRICS` composite
  whose component ran `SHZ` translated whole by 15/64 px. `first_point` is now carried explicitly
  (`HC_FIRSTPT`, the free slot at 824 — no byte pin moved). `SHZ` is live in MonofurNerdFont's 561
  and DejaVuSansM's 12 simple-glyph programs; no swept host face happened to observe it.
- **A zero-length `vmtx` counted as present**, giving every glyph tsb = ah = 0; FreeType's table
  lookup treats a zero-length entry as missing (ref2143/ttload.c:80-88) and the vertical phantoms
  come from `OS/2` or `hhea`. A glyph program reading pp3 saw 320 against FreeType's 512 at 12 ppem
  (384 against 704 at 16). `src/vert.cyr`'s resolve drops such an entry at open too.
- **The hinted left phantom applied the HVAR lsb delta.** FreeType's TrueType driver leaves
  `lsb_adjust` NULL (ref2143/ttdriver.c:559) and its pp1 is xMin − the UNVARIED bearing; rekha's
  point 0 sat at 165 against FreeType's 101 at wght 900, and its own hinted and unhinted paths for
  the same glyph disagreed by 1 px. The phantom follows FreeType now; `rekha_left_side_bearing`
  keeps the delta (its frozen semantics match `rekha_advance_width`'s).
- **An axis change did not re-run `prep`.** `rekha_hint_ctx` rebuilt the size only on a ppem
  change, so a `prep` branching on `GETINFO`'s variation bit kept the previous instance's state
  (storage 0 stayed 0 where the new instance writes 1,024). The setters now mark the context stale
  (`rekha_hint_axes_changed`) and the next `rekha_hint_ctx` re-runs it.
- `rekha_hint_vlen`'s `shift < 0` tail is 32-bit as `FT_Vector_Length`'s is (unreachable from a
  2x2, but a verbatim port is verbatim). `rekha_hint_load_simple`, the test seam, no longer rounds
  an empty gid's phantoms. `rekha_hint_present`'s comment said glyph programs count; they do not
  (*Divergences*).

### Bounded

Still one `sd_alloc` per font: **36,384 B** for the embedded face (0.8.1: 32,888) and **7,624 B**
for the suite's synthetic face (0.8.1: 7,040), plus **8,960 B** for unit font 0 and **7,832 B** for
the synthetic `glyf` face, all four asserted with the arithmetic in the comment. The header is **648 -> 832 B** (the
ten-field snapshot, the metric scale, the size's and the load's `cvt ` and storage pointers, the
running twilight cap, the 48-byte outline view, `first_point` in the last free slot); the carve
adds a second copy of the control values and the storage area for the glyph range and the on-curve
bytes. `REKHA_FONT_SIZE` is **unchanged at 832**. **No per-glyph allocation**: zone 1 is the
outline, composites recurse in place by rewriting the zone descriptor, the component records are
re-walked from the `glyf` bytes, and the load context lives on the stack. The caps are the outline
loader's — 64 loads, 16,384 points, depth 5, a cycle — plus the zone's own capacity from `maxp`,
under `REKHA_HINT_MAXZONE` 4,096. `src/hint.cyr` is **4,663 lines, 161 functions, 33 `@public`
comments** (0.8.1: 3,266 / 123 / 26).

### ⚠ Divergences from FreeType, stated at their sites and pinned in the roadmap

- ⚠ **rekha REFUSES what FreeType's default mode reads as 0 or skips — and the pin's "revisit"
  trigger has now fired**, at 0.61% of the four-ppem corpus and 29.8% of AdwaitaMono at 9 of its
  25 ppems (above). **Kept for 0.8.2, as a decision and not an oversight**: a glyph refused this
  way is drawn scaled and unhinted, never a plausible wrong glyph, and every refusal names its
  instruction through `rekha_hint_error`. The alternative is named so the freeze can choose it:
  mirror FreeType's default-mode fallbacks IN THE GLYPH RANGE ONLY — `RCVT` past the table reads 0
  (src/hint.cyr `rekha_hint_op_mem`), a short stack reads zeros (`rekha_hint_need`), a bad point is
  skipped (`rekha_hint_pt_ok`) — which a consumer asking for it reopens.
...
- ⚠ **`SHZ` in a NESTED composite's own program** walks `first_point` slots past its last contour,
  as FreeType's `Ins_SHZ` does (it never subtracts `first_point`, ref2143/ttinterp.c:5139) — but
  rekha CLAMPS at the sub-zone's end where FreeType writes past its points array: an out-of-bounds
  write that on the host corrupts `libfreetype`'s heap or the adjacent tag bytes (the
  `comp_nested_first_point` row was generated inside that regime and says so; `comp_nested_shz_low`
  pins the rule with `first_point` 3, inside the allocation). The two agree wherever the reference
  survives.
- ⚠ **FreeType's composite recursion cap of 100** is not modelled; rekha's caps are the outline
  loader's and `maxp`'s zone capacity — a glyph past its own `maxp` maxima is refused where
  FreeType, which sizes nothing from those fields, renders it (0 of 1,129 host faces).
- ⚠ **The scan-type bits** FreeType ORs into the first tag byte of every instructed glyph (4 |
  `scan_type << 5`) are not kept; the oracle masks with 0x19.
- ⚠ **`hdmx` is not read**, and `rekha_hint_advance_px` is where it would matter: on an `hdmx` face
  at a listed ppem FreeType reports the table's byte as a hinted slot's advance
  (ref2143/ttgload.c:2302-2313, :1974-1975); rekha answers the hinted phantom distance, equal only
  when the table is a faithful cache. Measured on a synthetic face (4 of 4 advances differ at the
  listed ppem, 0 elsewhere); no host face carries one.
- ⚠ **Variable instances are hinted off-reference**, and the default location is bit-exact: no
  `cvar`; the outline's deltas rounded per tuple and scaled from whole units where FreeType rounds
  once on 16.16; `prep` re-run on EVERY axis change where FreeType re-runs it only when the face
  has a `cvar` (ref2143/ttgxvar.c:3993-3997) and otherwise keeps the previous instance's state
  until the size is requested again; `GETINFO`'s variation bit and `GETVARIATION`'s gate are an
  axis actually off its default, where FreeType's flag stays set once any variation call was made,
  even back at the defaults; and the lsb map applies to `rekha_left_side_bearing` and NOT to the
  hinted pp1, so on such an instance the metric reader and the hinted outline's origin differ by
  the delta.
- ⚠ **A `REPEAT` flag count past the last point is CLAMPED** in the shared decoder — a leniency
  the other way round: FreeType refuses the glyph as `Invalid_Outline` (ref2143/ttgload.c:451-452)
  where rekha draws and hints it. Predates 0.8.2; the hinted and unhinted loaders agree with each
  other.
- ⚠ **Three `loca` rules FreeType sanitizes and rekha refuses** (`rekha_glyf_span`, against
  `tt_face_get_location`, ref2143/ttpload.c:183-288): offsets that run backwards (FreeType reads
  to the end of `glyf`; rekha `"loca order"`), the LAST glyph's end past `glyf` (FreeType clips;
  rekha `"loca past glyf"`), any other glyph's end or start past `glyf` (FreeType loads an empty
  glyph with real phantoms; rekha the same refusal). With the `REPEAT` clamp, these are the whole
  of the corpus's `refused` class on a face pedantic FreeType loads — and it is empty.
- ⚠ **An `OS/2` shorter than 78 bytes is absent** to rekha (so `hhea` decides the vertical
  phantoms) where FreeType reads its 78 field bytes from the stream regardless and takes the
  `OS/2` branch with whatever follows. rekha's gate is the saner rule and is kept.
- ⚠ **Glyph-only bytecode is not run**: `rekha_hint_present` gates on `fpgm` / `prep` / `cvt `
  with a non-zero length, so a face whose only programs are in its glyphs answers `NO_TABLE
  "hint tables"` and draws unhinted. That is FreeType's DEFAULT load — it hands a face with no
  `fpgm` and a `prep` of at most 7 bytes to the autohinter (ref2143/ftobjs.c:1006-1016) — not its
  `NO_AUTOHINT` one, and rekha has no autohinter to fall back to.
- ⚠ A composite's `n_ins` or bytes past the glyph's `loca` span refuse `TRUNCATED "composite
  instructions"` where FreeType reads on from the stream; the per-run twilight bound has no +4
  band and is the running MINIMUM within one load; org is filled and the phantom tags are 0 even
  when no program runs (FreeType leaves both stale, unobservably).
- ⚠ **CI's FreeType is still 2.13.2.** Neither generated module is regenerated there and none of
  the six hinting scripts under `scripts/` runs there; both modules are regenerated by hand on a
  2.14.x host and reviewed as a diff. `scripts/ftshim.py` refuses any FreeType but 2.14.x.

`programs/hint_test.cyr` — **109,221 checks** over sixteen groups A..P (0.8.1: 7,387). New: **P**
the 233 FreeType-recorded glyphs; **K** the 167 unit programs, every row three times — 60,121 of
the checks are the consumer-flow pass, which on the one row that carries twilight state
(`deltap_sdb_sds`) fails when run on a context one load ahead and passes on a fresh one, so it
sees the +64 it was written to see; **L glyph** the synthetic `glyf` face driving every refusal
class, the per-load copy shared across a composite, `WCVTF` write-through, the snapshot, `INSTCTRL`
bits 1 / 2 / both, the size-change pin, the twilight running minimum, the zone cap, the tail rule,
the unvaried left phantom of an HVAR instance (point 0 at 101, FreeType-measured), the `prep`
re-run after an axis change, a zero-length `vmtx` and a simple component's `SHZ` under
`USE_MY_METRICS`; **O** the 0-byte pins; **J**'s two `IUP` refusals flipped to runs.
`programs/hostile_test.cyr` — **563 checks**: the hinted path is in the per-font sentinel digest
and the mutation loop (**240,908 hinted loads over 120 mutants**, A/B digests equal, a refused
mutant answers 0 and an empty view, never a fault), 2,620 hinted glyphs pinned for the face at 12
ppem, the hinting family on null handles and under a refusing hook; the run is **8.95 s** (0.8.1:
1.85 s), since every gid of every sweep is hinted. The verification round's own sweep — the 25
unit fonts under a full single-byte and u16 mutation of every offset, **~85M checks** under a
guard-page allocator with canaries, 75,000 random glyph programs through the composite loader,
400 face mutants and all 1,750 host TTFs — found **0 faults, 0 canary writes and 0 broken refusal
invariants**.
